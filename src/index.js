...
import createImageCard from './tpl/img-card.hbs';
import '@pnotify/core/dist/PNotify.css';
import '@pnotify/core/dist/BrightTheme.css';
import '@pnotify/mobile/dist/PNotifyMobile.css';
import '@pnotify/countdown/dist/PNotifyCountdown.css';
import { alert } from '@pnotify/core';
import optNotify from './js/setNotification.js';
...
async function onSearch(e) {
  ...
  if (fetchTotalHits === 0) {
    return alert(optNotify.incorrectQuery);
  }

  if (apiService.query === '') {
    helpFoundInput(e);
    return alert(optNotify.notMachResults);
  }
  if (fetchTotalHits > 0) {
    (optNotify.successResult.title = `Found ${fetchTotalHits} ${apiService.query} images`),
      alert(optNotify.successResult);
  }
}
